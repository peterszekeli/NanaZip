...
1. Replace worker-thread `SendMessage()` with `PostMessage()` (or use `SendMessageTimeout()` with a fixed 1500 ms timeout and a safe fallback path); 1500 ms is a practical upper bound to avoid prolonged UI stalls while still allowing short UI-handler bursts.
...
2. Remove `_numActiveThreads` and derive the active count from the lock-protected thread container (for example, count entries where `Thread_WasCreated()` is true) to keep one authoritative source of truth.
...
- `PanelItemOpen.cpp:1280-1286` (loop breaks when `handles.Size() > 60`; this is near the Win32 `MAXIMUM_WAIT_OBJECTS` limit of 64 because one slot is reserved for the exit event and extra headroom is retained)
